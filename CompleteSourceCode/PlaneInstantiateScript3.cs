...
// PlaneInstantiateScript3 to instantiate the plane object in the waves
public class PlaneInstantiateScript3 : MonoBehaviour {
	...
	IEnumerator PlaneCreation(float waitTime){
		for (int y = 2; y < 5; y++) {
			for (int x = 31; x < 45; x++) {
				...
				Instantiate(planePrefab, new Vector3(x+3, y+3, 10), Quaternion.identity);
